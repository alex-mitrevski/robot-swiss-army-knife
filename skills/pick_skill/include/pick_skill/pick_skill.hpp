...
/**
 * A configurable object grasping skill.
 * The skeleton of the skill's lifecycle node was created with PAL's rpk utility.
 *
 * @author Alex Mitrevski
 * @contact alemitr@chalmers.se
 */
class PickSkillNode : public rclcpp_lifecycle::LifecycleNode
{
public:
    using LifecycleCallbackReturn = rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn;
    using PickSkill = pick_skill::action::PickSkill;
    using PickSkillGoal = std::shared_ptr<const PickSkill::Goal>;
    using PickSkillGoalHandle = std::shared_ptr<rclcpp_action::ServerGoalHandle<PickSkill>>;

    explicit PickSkillNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions());
    ~PickSkillNode();
...
private:
     ...
     * - ...
     */
    void execute_skill(const PickSkillGoalHandle goal_handle);

    /**
     * Accepts the goal if the node is active; rejects the goal otherwise.
     * Both method arguments are unused.
     */
    rclcpp_action::GoalResponse request_goal_cb(const rclcpp_action::GoalUUID &, PickSkillGoal);
     ...
     */
    void request_accepted_cb(const PickSkillGoalHandle goal_handle);
     ...
     * force sum (along z) for surface detection.
     *
    ...
    rclcpp_action::Server<PickSkill>::SharedPtr skill_server;
    ...
    unsigned int min_wrench_measurements_for_grasp_detection;
